...
public class RedisUtil {
    ...
    @Value("${token.expirationSeconds:100000}")
    private int expirationSeconds;

    /*常量，各种实现方式都行，这里读取application.yml*/
    @Value("${token.validTime:200000}")
    ...
    public Boolean isBlackList(String token){
        Boolean flag = hasKey("blacklist", token);
        System.out.println(flag);
        return flag;
    }
    ...
    public Object getRoleByToken(String token) {
        return redisTemplate.opsForHash().get(token, "role");
    }
    ...
    public void setTokenRefresh(String token,String username,String ip,String role,Integer expire){
        //刷新时间
        //Integer expire = validTime*24*60*60*1000;

        hset(token, "tokenValidTime",DateUtil.getAddDaySecond(60*30),expire);  //半小时过期
        hset(token, "expirationTime",DateUtil.getAddDaySecond(expirationSeconds),expire);  //5风中token失效
        hset(token, "username",username,expire);
        hset(token, "role",role,expire);
