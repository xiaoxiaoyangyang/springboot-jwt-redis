...
//                    .setSigningKey(salt) // 不使用公钥私钥
    ...
    public static void main(String[] args){
        ...
        String token = generateToken("baixianlong", 3600, claims);
        System.out.println("token:"+token);
        String name = parseToken(token, null);
        System.out.println("name:"+name);
        Map<String, Object> claims1 = getClaims(token);
        String role = claims1.get("role").toString();
